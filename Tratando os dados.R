...
for (ano in 2011:2021) {
  #arquivo <- data.frame()
  #  for (nome in nomes) { 
  hist_dre <- rbind(hist_dre, read_csv2(paste("dados_CVM_csv/itr_cia_aberta_", nome,"_", ano, ".csv", sep = ""),
...
hist_reclam_2$Categoria_2 <- replace(hist_reclam_2$Categoria_2, hist_reclam_2$Categoria_2 == "FALSE", "Menos de 4 milhoes de clientes")
hist_reclam_2$Categoria_2 <- replace(hist_reclam_2$Categoria_2, hist_reclam_2$Categoria_2 == "TRUE", "Mais de 4 milhoes de clientes")
